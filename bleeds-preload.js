...
(function () {
    ...
    const _globalPathSet = new Set(Module.globalPaths);
    ...
    Module._resolveLookupPaths = function (request, parent) {
        if (parent && !_addedParents.has(parent)) {
            ...
            if (!parent.paths?.length) parent.paths = [..._globalPathSet];
            else for (const p of _globalPathSet) if (!parent.paths.includes(p)) parent.paths.push(p);
        }
...
const r = (e, ...o) => ipcRenderer.invoke(e, ...o);
const T = (e, ...o) => ipcRenderer.sendSync(e, ...o);
...
if (location.protocol !== "data:") {
    try { r("VencordInitFileWatchers"); } catch { }

    try {
        const rendererJs = T("VencordPreloadGetRendererJs");
        if (rendererJs) webFrame.executeJavaScript(rendererJs).catch(e => console.error("[Bleeds Client] renderer inject failed:", e?.message));
    } catch (e) {
        console.error("[Bleeds Client] VencordPreloadGetRendererJs failed:", e);
    }

    if (process.env.DISCORD_PRELOAD) {
        try { require(process.env.DISCORD_PRELOAD); } catch { }
    }
