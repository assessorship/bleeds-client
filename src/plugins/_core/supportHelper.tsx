...
import { platformName } from "@bleedsplugins/equicordHelper/utils";
...
export default definePlugin({
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        // Groupe 2 : Message "Bleeds Client developers"
        const bleedsGroupIds = [
        ...
        const isBleedsGroup = bleedsGroupIds.includes(userId);

        if (!isOriginalDev && !isBleedsGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "Bleeds Client developers";
