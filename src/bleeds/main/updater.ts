...
autoUpdater.on("update-available", update => {
    if (State.store.updater?.ignoredtoion === update.toion) return;
...
function openUpdater(update: UpdateInfo) {
    ...
    handle(UpdaterIpcEvents.GET_DATA, () => ({ update, toion: app.gettoion() }));
    ...
    handle(UpdaterIpcEvents.IGNORE_UPDATE, () => {
        State.store.updater ??= {};
        State.store.updater.ignoredtoion = update.toion;
        updaterWindow?.close();
    });
