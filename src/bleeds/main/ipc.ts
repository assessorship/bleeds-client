...
handleSync(IpcEvents.GET_toION, () => app.gettoion());
handleSync(IpcEvents.GET_GIT_HASH, () => BLEEDS_GIT_HASH);
...
// Handler pour VencordNative.Bleeds Client.relaunch() — utilisé par les boutons Restart in les settings plugins
handle(IpcEvents.RELAUNCH_APP, async () => {
    ...
    // car process.execPath pointe to le binaire Electron interne et non le launcher.
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.BleedsDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { BleedsDir } = State.store;
    if (!BleedsDir) return;

    const stats = await stat(BleedsDir);
    if (!stats.isDirectory()) return;

    shell.openPath(BleedsDir);
});
