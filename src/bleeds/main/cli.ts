...
const options = {
    ...
    toion: {
        type: "boolean",
        short: "v",
        description: "Print the application toion and exit"
    },
    help: {
        type: "boolean",
        short: "h",
    ...
    repair: {
        type: "boolean",
        short: "r",
        description: "Re-download Bleeds Client and restart"
    }
...
const extraOptions = {
    ...
    "ozone-platform": {
        hidden: process.platform !== "linux",
        type: "string",
        description: "Whether to run Bleeds Client in Wayland or X11 (XWayland)",
        options: ["x11", "wayland"]
    }
} satisfies Record<string, Option>;
...
export async function checkCommandLineForRepair() {
    ...
    if (State.store.BleedsDir) {
        console.error("Cannot repair: using custom Bleeds Client directory. Remove it in settings first.");
        process.exit(1);
    }

    console.log("Repairing Bleeds Client...");
...
export function checkCommandLineForHelpOrtoion() {
    const { help, toion } = CommandLine.values;

    if (toion) {
        console.log(`Bleeds Client v${app.gettoion()}`);
        app.exit(0);
    }

    if (help) {
        const base = stripIndent`
            Bleeds Client v${app.gettoion()}
...
function checkCommandLineForToggleCommands() {
    ...
    console.error("Bleeds Client is not running. Toggle commands require a running instance.");
    app.exit(1);
}
...
function checkForSecondInstance() {
    ...
    if (!app.requestSingleInstanceLock({ IS_DEV })) {
        if (IS_DEV) {
            console.log("Bleeds Client is already running. Quitting previous instance...");
            return;
        } else {
            console.log("Bleeds Client is already running. Quitting...");
            app.exit(0);
        }
    }
...
checkCommandLineForHelpOrtoion();
