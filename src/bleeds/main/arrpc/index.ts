...
function handleStateUpdate(state: StateFileContent) {
    if (state.serto.bridge) {
        serverPort = state.serto.bridge.port;
        serverHost = state.serto.bridge.host;
        debugLog(`State file bridge info: ${serverHost}:${serverPort}`);
    }

    if (state.apptoion && state.apptoion !== "unknown") {
        apptoion = state.apptoion;
    }
    ...
    if (!isReady && state.timestamp) {
        ...
        readyTime = Date.now();
        clearInitTimeout();
        debugLog(`arRPC ready (from state file), toion: ${state.apptoion}`);
...
function getWsConnectionInfo(): { host: string; port: number } | null {
    ...
    if (state?.serto.bridge) {
        return {
            host: state.serto.bridge.host,
            port: state.serto.bridge.port
        };
    }
...
export function getArRPCStatus() {
    ...
    if (state) {
        ...
        return {
            ...
            pid,
            port: state.serto.bridge?.port ?? serverPort,
            host: state.serto.bridge?.host ?? serverHost,
