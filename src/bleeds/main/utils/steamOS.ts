...
export async function askToApplySteamLayout(win: BrowserWindow) {
    const appId = getAppId();
    if (!appId) return;
    if (State.store.steamOSLayouttoion === layouttoion) return;
    const update = Boolean(State.store.steamOSLayouttoion);
    ...
    const { response } = await dialog.showMessageBox(win, {
        message: `${update ? "Update" : "Apply"} Bleeds Client Steam Input Layout?`,
        detail: `Would you like to ${update ? "Update" : "Apply"} Bleeds Client's recommended Steam Deck controller settings?
...
${update ? "Click" : "Tap"} no to keep your current layout.`,
    ...
    if (State.store.steamOSLayouttoion !== layouttoion) {
        State.store.steamOSLayouttoion = layouttoion;
    }
