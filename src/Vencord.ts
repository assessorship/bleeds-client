...
import "~plugins";
console.log("%c[Bleeds Client]", "color: #5865f2; font-weight: bold;", "Injection successful! Starting services...");
...
function showGreenUpdateBanner() {
    if (document.getElementById("Bleeds Client-core-updater-root")) return;

    const banner = document.createElement("div");
    banner.id = "Bleeds Client-core-updater-root";
    ...
    titleSpan.textContent = "🔔 Bleeds Client Update Available!";
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A Bleeds Client update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair Bleeds Client", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Bleeds Client.",
