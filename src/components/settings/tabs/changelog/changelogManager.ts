...
const BLEEDS_RELEASES_REPO = "bleedsclient/bleeds-client";
const BLEEDS_REPO_URL = `https://github.com/${BLEEDS_RELEASES_REPO}`;
...
export { BLEEDS_REPO_URL };

export async function getCommitsSinceLastSeen(
    repoUrl: string,
): Promise<ChangelogEntry[]> {
    // Toujours utiliser le repo Bleeds Client, ignorer le repoUrl d'Equicord
    return fetchCommitsBetween(BLEEDS_RELEASES_REPO, "HEAD~10", "HEAD").catch(() => []);
}
