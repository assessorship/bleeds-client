...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of Bleeds Client"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const [searchValue, setSearchValue] = useState({ value: "", status: SearchStatus.BLEEDS });
    ...
    React.useEffect(() => {
        ...
        window.addEventListener("Bleeds Client-tutorial-detected", handler);
        return () => window.removeEventListener("Bleeds Client-tutorial-detected", handler);
    }, []);
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.BLEEDS:
                if (!PluginMeta[plugin.name].folderName.startsWith("src/bleedsplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (PluginMeta[plugin.name].folderName.startsWith("src/bleedsplugins/") || PluginMeta[plugin.name].folderName.startsWith("src/plugins/_")) return false;
    ...
    const { bleedsPlugins, othersPlugins, requiredPlugins } = useMemo(() => {
        const bleedsPlugins = [] as JSX.Element[];
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                const tooltipText = p.required || !depMap[p.name]
                    ? "This plugin is required for Bleeds Client to function."
                ...
                const isBleedsPlugin = folderName.startsWith("src/bleedsplugins/");
                const card = (
                    <PluginCard
                        onRestartNeeded={handleRestartNeeded}
                        disabled={false}
                ...
                if (isBleedsPlugin) {
                    bleedsPlugins.push(card);
                } else {
                    othersPlugins.push(card);
                }
            }
        }
        return { bleedsPlugins, othersPlugins, requiredPlugins };
    ...
    const allPlugins = [...bleedsPlugins, ...othersPlugins];
    ...
    const bleedsVisible = bleedsPlugins.slice(0, Math.min(visibleCount, bleedsPlugins.length));
    const othersVisible = othersPlugins.slice(0, Math.max(0, visibleCount - bleedsPlugins.length));
        ...
        <SettingsTab>
                    ...
                    <ErrorBoundary noop>
                        <Select
                            options={[
                                ...
                                { label: "Show Bleeds Client Plugins", value: SearchStatus.BLEEDS },
            ...
            {premiumOnly ? (
                <>
                    ...
                    {bleedsPlugins.length || othersPlugins.length
                        ? (
                            <div className={cl("grid")}>
                                {[...bleedsVisible, ...othersVisible].length
                                    ? [...bleedsVisible, ...othersVisible]
                ...
                <>
                    {bleedsPlugins.length > 0 && (
                        <>
                            <HeadingTertiary className={Margins.top20}>Bleeds Client Plugins</HeadingTertiary>
                            <div className={cl("grid")}>
                                {bleedsVisible}
                            </div>
                        </>
                    ...
                    {bleedsPlugins.length === 0 && othersPlugins.length === 0 && (
                        <ExcludedPluginsList search={search} />
                    )}
