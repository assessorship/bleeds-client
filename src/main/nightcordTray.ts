/*
 * Bleeds Client — System Tray (injection mode)
 * Creates a Bleeds Client system tray icon when injected into Discord Desktop.
...
/**
 * Initializes the Bleeds Client system tray icon.
 ...
 */
export function initBleedsTray(injectorPath: string) {
    ...
    app.whenReady().then(() => {
        try {
            ...
            tray.setToolTip("Bleeds Client");

            const trayMenu = Menu.buildFromTemplate([
                {
                    label: "Open Bleeds Client",
                    click() {
                ...
                { type: "separator" },
                {
                    label: "Uninstall Bleeds Client",
                    click() {
                        const uninstallerCandidates = [
                            join(process.env.LOCALAPPDATA || "", "Programs", "Bleeds Client", "Uninstall Bleeds Client.exe"),
                            join(process.env.LOCALAPPDATA || "", "Programs", "Bleeds Client", "Uninstall Bleeds Client.exe"),
                            join(process.env.PROGRAMFILES || "", "Bleeds Client", "Uninstall Bleeds Client.exe"),
                        ];
            ...
            console.log("[Bleeds Client] Tray created ✓", iconPath ?? "(icon from exe)");
        } catch (e) {
            console.error("[Bleeds Client] Failed to create tray:", e);
        }
    });
}
