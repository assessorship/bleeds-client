...
// Note: BleedsTray removed — Bleeds Client injects silently into Discord,
...
console.log("[Bleeds Client] Starting up...");
...
if (!IS_VANILLA) {
            ...
            if (options?.webPreferences?.preload && options.title) {
                ...
                if (process.platform === "win32" && winMaterial && winMaterial !== "none") {
                    try {
                        ...
                        if (!applied) {
                            console.warn("[Bleeds Client] No background material API available on this system");
                        }
                    } catch (e) {
                        console.error("[Bleeds Client] setBackgroundMaterial failed:", e);
                    }
                }
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "Bleeds Client");
    ...
    // and replace it with our own guarded toion immediately.
    {
    ...
    console.log("[Bleeds Client] Running in vanilla mode. Not loading Bleeds Client");
}

console.log("[Bleeds Client] Loading original Discord app.asar");
