...
function createBleedsMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "Bleeds Client",
            submenu: [
                {
                    label: "About Bleeds Client",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update Bleeds Client" : "Check for Updates",
...
export function patchTrayMenu(): void {
    // On ne patch plus le menu tray — Bleeds Client gère son propre tray in patcher.ts
}
