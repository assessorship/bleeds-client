 ...
 * the Free Software Foundation, either toion 3 of the License, or
 * (at your option) any later toion.
 *
...
// On regroupe tout in une seule commande PS pour la performance et la fiabilité
    ...
    return new Promise<void>((resolve, reject) => {
        ...
        try {
            if (hWnd && hWnd.length >= 4) {
                ...
                // On lit comme uint32 les 4 bytes bas (les HWND Windows restent in 32 bits en pratique)
...
<script>
    ...
    function processSearch() {
        ...
        if (validWords.length === 0) {
            document.getElementById('status').innerText = "No mot found !";
            document.getElementById('status').style.color = "#ef4444";
            return;
        }
        ...
        if (safeMode) {
            ...
            if (!groqKey) {
                defText.innerText = "Erreur: Clé API Groq not found.";
            } else {
...
ipcMain.handle(IpcEvents.OPEN_MONACO_EDITOR, async () => {
    ...
    monacoWin = new BrowserWindow({
        title: "Bleeds Client QuickCSS Editor",
...
// ── Bleeds Client Relaunch — redémarre l'app Electron complètement ──
// VencordNative.Bleeds Client.relaunch() envoie RELAUNCH_APP = "BleedsRelaunchApp"
// VesktopNative.app.relaunch() envoie aussi RELAUNCH_APP
// Les boutons "Restart" in les plugins settings (index.tsx) passent par VencordNative.Bleeds Client.relaunch()
ipcMain.handle(IpcEvents.RELAUNCH_APP, async () => {
    // in l'architecture Bleeds Client, app.asar est un dossier (pas un vrai .asar),
...
// ── Bleeds Client Updater — downloads a Setup.exe from a URL and runs it ──
ipcMain.handle(IpcEvents.BLEEDS_DOWNLOAD_AND_RUN, async (_, url: string) => {
    ...
    const tmpPath = path.join(os.tmpdir(), "BleedsUpdate-Setup.exe");
...
ipcMain.handle(IpcEvents.INSTALL_VB_CABLE, async () => {
    ...
    const tmpDir = path.join(os.tmpdir(), "Bleeds Client-VBCable");
    ...
    return new Promise((resolve) => {
        // Commande PowerShell : Downloading -> Extraction -> Execution de l'installeur (x64 car Bleeds Client est x64)
        const psCmd = `
        ...
        exec(`powershell -Command "${psCmd}"`, (err: any) => {
            if (err) {
                console.error("[Bleeds Client] VBCable install failed:", err);
