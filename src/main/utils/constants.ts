...
export const DATA_DIR = process.env.BLEEDS_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "BleedsData", suffix)
        : join(app.getPath("userData"), "..", "Bleeds Client", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Bleeds Client] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
