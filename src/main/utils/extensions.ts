...
export async function installExt(id: string) {
    ...
    try {
        ...
        const url = `https://clients2.google.com/service/update2/crx?response=redirect&acceptformat=crx2,crx3&x=id%3D${id}%26uc&prodtoion=${process.toions.chrome}`;

        const buf = await fetchBuffer(url, {
            headers: {
                "User-Agent": `Electron ${process.toions.electron} ~ Bleeds Client (https://github.com/bleedsclient/bleeds-client)`
            }
        });
