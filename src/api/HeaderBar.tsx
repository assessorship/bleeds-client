...
try { _stealthActive = localStorage.getItem("BLEEDS_stealthMode") === "1"; } catch { }
...
function persistStealth(v: boolean) {
    try { v ? localStorage.setItem("BLEEDS_stealthMode", "1") : localStorage.removeItem("BLEEDS_stealthMode"); } catch { }
}

// Éléments NON-React uniquement (titlebar, bleedsAI nav)
// NE PAS cacher les entrées settings sidebar — l'utilisateur doit pouvoir accéder aux paramètres
const NON_REACT_SELECTORS = [
    "#Bleeds Client-titlebar-btn",
    "#Bleeds Client-titlebar-link-style",
    ".nai-nav-item",
];
...
export function syncStealthBodyClass() {
    try { if (_stealthActive) document.body?.classList.add("Bleeds Client-stealth"); else document.body?.classList.remove("Bleeds Client-stealth"); } catch { }
...
if (_stealthActive) {
    ...
    try { document.body?.classList.add("Bleeds Client-stealth"); } catch { }
}
...
try {
    ...
    window.addEventListener("Bleeds Client-stealth-change", () => {
...
export function _notifyStealthChange() {
    ...
    window.dispatchEvent(new Event("Bleeds Client-stealth-change"));
}
...
function HeaderBarButtons() {
    ...
    useEffect(() => {
        ...
        window.addEventListener("Bleeds Client-stealth-change", listener);
        return () => {
            headerBarListeners.delete(listener);
            stealthListeners.delete(listener);
            window.removeEventListener("Bleeds Client-stealth-change", listener);
        };
    }, []);
