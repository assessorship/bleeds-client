...
import { EQUIBOT_USER_ID, BLEEDS_BOT_USER_ID } from "@utils/constants";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of Bleeds Client. Try updating!";
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    // Process components — Bleeds Bot (bleedsclient.xyz, Component v2 Container format)
    if (message.author.id === BLEEDS_BOT_USER_ID) {
        ...
        if (containerComponents?.length >= 3) {
            ...
            if (pluginUrl?.startsWith("https://bleedsclient.xyz/plugins/")) {
