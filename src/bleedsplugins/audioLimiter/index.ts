...
function getUserStore() {
    if (!_userStore) _userStore = (window as any).Vencord?.Webpack?.findByStoreName?.("UserStore") ?? (window as any).Vencord?.Webpack?.common?.UserStore;
    return _userStore;
}
...
function applyLimiter(data: StreamData) {
    ...
    const myId = getUserStore()?.getCurrentUser?.()?.id;
