...
import { t, useTranslation } from "../autoTranslateBleeds";
import "./styles.css";
...
function loadPersistLogs() {
    try {
        const s = localStorage.getItem("BLEEDS_logs");
        if (s) {
...
function savePersistLogs() {
    try {
        ...
        localStorage.setItem("BLEEDS_logs", JSON.stringify(toSave));
    } catch { }
}
...
function LogRow({ e }: { e: LogEntry; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("Bleeds Client Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `BLEEDS_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
...
export default definePlugin({
    ...
    start() {
        ...
        loadPersistLogs();
        addHeaderBarButton("Bleeds Client-event-logs", () => <LogsButton />, 7);
        subscribeToEvents();
    },
    stop() {
        removeHeaderBarButton("Bleeds Client-event-logs");
