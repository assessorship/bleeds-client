/*
 * Bleeds Client — AntiDeleteMessage
...
function getBlacklistedGuildIds(): Set<string> {
    if (!blacklistCache) {
        ...
        blacklistCache = new Set(raw.split(",").map(s => s.trim()).filter(Boolean));
    }
...
function addToCache(messageId: string, data: CachedMessage) {
    ...
    if (ids.length >= maxSize) {
        ...
        for (let i = 0; i < toDelete; i++) delete memCache[ids[i]];
    }
