...
function getRandomSound(soundsArray: { playing: boolean; player: AudioPlayerInterface; }[]) {
    const nonplaying = soundsArray.filter(s => !s?.playing);
    const pool = nonplaying.length ? nonplaying : soundsArray;
    const chosen = pool[Math.floor(Math.random() * pool.length)];
    if (chosen) { chosen.playing = true; chosen.player.restart(); }
}

let assignDebounceTimer: ReturnType<typeof setTimeout> | null = null;
function debouncedAssignSounds(volume: number, packId: string) {
    if (assignDebounceTimer) clearTimeout(assignDebounceTimer);
    assignDebounceTimer = setTimeout(() => assignSounds(volume, packId), 150);
}
...
const settings = definePluginSettings({
    volume: {
        ...
        default: 120,
        max: 200,
        onChange: value => { debouncedAssignSounds(value, settings.store.soundPack); }
    },
    soundPack: {
