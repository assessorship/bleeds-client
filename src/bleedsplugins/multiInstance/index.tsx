/*
 * Bleeds Client — MultiInstance plugin
 *
...
import { t } from "../autoTranslateBleeds";
import "./styles.css";
...
const MI_TOKEN_CACHE_KEY = "Bleeds Client-mi-token-cache";
...
function ContextMenuPortal(props: CtxMenuProps) {
    ...
    const [container] = React.useState(() => {
        const el = document.getElementById("Bleeds Client-mi-ctx-root") ?? document.createElement("div");
        el.id = "Bleeds Client-mi-ctx-root";
...
export default definePlugin({
    ...
    async start() {
        ...
        captureCurrentToken();
        addHeaderBarButton("Bleeds Client-multi-instance", () => <MultiInstanceButton />, 9);
    },

    stop() {
        removeHeaderBarButton("Bleeds Client-multi-instance");
        if (this._fluxUnsub) { this._fluxUnsub(); this._fluxUnsub = null; }
        const root = document.getElementById("Bleeds Client-mi-ctx-root");
        if (root) root.remove();
    },
});
