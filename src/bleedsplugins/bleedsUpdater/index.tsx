/*
 * Bleeds Client — Auto-updater plugin
 * Checks GitHub on startup and shows a green banner if a newer version exists.
 */
...
const REMOTE_VERSION_URL =
    "https://api.github.com/repos/bleedsclient/bleeds-client/releases/latest";

// ── Local version (injected at build time via define) ─────────────────────────
...
// ── Semver comparison: true only if remote > local ────────────────────────────
...
async function checkForUpdates() {
    try {
        ...
        if (!res.ok) return; // no releases yet or private repo — silently skip

        const data = await res.json();
        if (!data?.tag_name) return;

        const remoteVersion: string = data.tag_name;
        console.log(`[bleedsUpdater] local=${localVersion} remote=${remoteVersion}`);

        if (isStrictlyNewer(remoteVersion, localVersion)) {
            pendingUpdate = { remoteVersion, localVersion, downloadUrl: "auto" };
            notify();
        }
    } catch {
        // network error or repo unavailable — silently ignore
    }
}
...
function UpdateBanner() {
    ...
    async function doUpdate() {
        ...
        try {
            ...
            console.error("[bleedsUpdater] Update error:", e);
    ...
    return React.createElement("div", {
        ...
        React.createElement("div", {
            ...
            React.createElement("span", { style: { fontWeight: 700, flexShrink: 0 } },
                `🔔 Bleeds Client ${info.remoteVersion} available!`
            ),
...
// ── Mount banner into DOM ─────────────────────────────────────────────────────
let bannerRoot: any = null;
let bannerContainer: HTMLDivElement | null = null;

function mountBanner() {
    if (bannerContainer || document.getElementById("bleeds-updater-root")) return;
    bannerContainer = document.createElement("div");
    bannerContainer.id = "bleeds-updater-root";
    (document.body ?? document.documentElement).appendChild(bannerContainer);
    try {
        ...
        console.error("[bleedsUpdater] Banner mount error:", e);
    }
}
...
export default definePlugin({
    ...
    description: "Checks for updates on startup. Green banner appears only if a newer version is available on GitHub.",
