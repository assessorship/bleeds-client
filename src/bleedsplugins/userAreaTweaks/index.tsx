...
export default definePlugin({
    name: "UserAreaTweaks",
    description: "Creates an elegant floating dock for Bleeds Client plugins using pure CSS, preventing overlap and crashes.",
    authors: [{ name: "Bleeds Client", id: 0n }],
    enabledByDefault: true,

    start() {
        const style = document.createElement("style");
        style.id = "Bleeds Client-userarea-tweaks-style";
        style.textContent = CSS;
        document.head.appendChild(style);
    },

    stop() {
        document.getElementById("Bleeds Client-userarea-tweaks-style")?.remove();
    }
});
