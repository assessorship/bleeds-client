# build-installer.ps1 - Compile Bleeds Client-Installer.exe
...
$OutExe = Join-Path $OutDir "Bleeds Client-Installer.exe"

Write-Host ""
Write-Host "  [Bleeds Client] Compiling installer..." -ForegroundColor Cyan
...
if ($hasDotnet) {
    ...
    & dotnet publish "$SrcDir\Bleeds ClientInstaller.csproj" `
        -c Release `
        -o $OutDir `
        --nologo `
        -v quiet `
    ...
    $built = Get-ChildItem $OutDir -Recurse -Filter "Bleeds Client-Installer.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
...
# Method 2: csc.exe (.NET Framework - always present on Windows)
    ...
    $ico    = Join-Path $Root "Bleeds Client.ico"
...
if (Test-Path $OutExe) {
    ...
    Write-Host ""
    Write-Host "  OK  Bleeds Client-Installer.exe compiled ($size KB)" -ForegroundColor Green
    Write-Host "    -> $OutExe" -ForegroundColor DarkGray
    Write-Host ""
} else {
    Write-Host "  [ERROR] Bleeds Client-Installer.exe not found after compilation." -ForegroundColor Red
    exit 1
}
