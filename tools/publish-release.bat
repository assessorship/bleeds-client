@echo off
:: ─── Bleeds Client — Publier une nouvelle release sur GitHub ─────────────────────
...
if "%NOTES%"=="" set NOTES=Bleeds Client %VERSION%

:: Chemins de sortie
set DIST_DIR=dist\desktop
set OUT_DIR=release\installer
set DIST_ZIP=%OUT_DIR%\bleeds-dist.zip
set INSTALLER_EXE=%OUT_DIR%\BleedsClient-Installer.exe
set VERSION_JSON=%OUT_DIR%\version.json
set DESKTOP_ASAR=dist\desktop.asar

echo.
echo  ╔═══════════════════════════════════════════════════╗
echo  ║    Bleeds Client — Publication release v%VERSION%
echo  ╚═══════════════════════════════════════════════════╝
echo.
...
:: ── 5. Compiler BleedsClient-Installer.exe ──────────────────────────────────────
echo.
echo  [5/8] Compilation de BleedsClient-Installer.exe...
...
if not exist "%INSTALLER_EXE%" (
    echo  [ERREUR] BleedsClient-Installer.exe introuvable apres compilation.
    pause
    exit /b 1
)

for %%F in ("%INSTALLER_EXE%") do echo  [5/8] BleedsClient-Installer.exe cree (%%~zF octets)

:: ── 6. Créer bleeds-dist.zip ──────────────────────────────────────────────
echo.
echo  [6/8] Creation de bleeds-dist.zip...
...
powershell -NoProfile -Command "Add-Type -Assembly System.IO.Compression.FileSystem; $src = (Resolve-Path '%DIST_DIR%').Path; $dst = (Join-Path (Resolve-Path 'release\installer').Path 'bleeds-dist.zip'); [System.IO.Compression.ZipFile]::CreateFromDirectory($src, $dst, [System.IO.Compression.CompressionLevel]::Optimal, $false)"

if not exist "%DIST_ZIP%" (
    echo  [ERREUR] Impossible de creer bleeds-dist.zip
    pause
    exit /b 1
)

for %%F in ("%DIST_ZIP%") do echo  [6/8] bleeds-dist.zip cree (%%~zF octets)
...
(
    ...
    echo   "installerUrl": "https://github.com/bleedsclient/bleeds-client/releases/latest/download/BleedsClient-Installer.exe",
    echo   "distUrl": "https://github.com/bleedsclient/bleeds-client/releases/latest/download/bleeds-dist.zip",
    echo   "downloadUrl": "https://github.com/bleedsclient/bleeds-client/releases/latest/download/desktop.asar",
...
gh release create "v%VERSION%" ^
    "%INSTALLER_EXE%#BleedsClient-Installer.exe" ^
    "%DIST_ZIP%#bleeds-dist.zip" ^
    "%DESKTOP_ASAR%#desktop.asar" ^
    "%VERSION_JSON%#version.json" ^
    --repo bleedsclient/bleeds-client ^
    --title "Bleeds Client v%VERSION%" ^
    --notes "!NOTES!" ^
    --latest
...
echo.
echo  ╔═══════════════════════════════════════════════════════════════╗
echo  ║  Bleeds Client v%VERSION% publie avec succes !
echo  ║
echo  ║  Fichiers publies sur GitHub :
echo  ║    BleedsClient-Installer.exe    — installeur .exe avec GUI
echo  ║    bleeds-dist.zip         — JS obfusques (pour l'injec.)
echo  ║    version.json               — metadonnees de version
echo  ║
echo  ║  Les utilisateurs telechargeront BleedsClient-Installer.exe
