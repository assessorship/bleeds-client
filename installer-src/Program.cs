...
namespace BleedsClientInstaller
{
    ...
    public class LauncherForm : Form
    {
        ...
        private BleedsClientBackend _backend;

        public LauncherForm()
        {
            this.Text = "Bleeds Client Installer";
            ...
            var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("BleedsClientInstaller.icon.ico");
        ...
        {
            var userDataFolder = Path.Combine(Path.GetTempPath(), "BleedsClientInstaller_WebView2");
            var env = await CoreWebView2Environment.CreateAsync(null, userDataFolder);
            await _webView.EnsureCoreWebView2Async(env);

            _backend = new BleedsClientBackend(this, _webView);
            ...
            using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("BleedsClientInstaller.index.html"))
            using (var reader = new StreamReader(stream))
            {
                ...
                try {
                    using (var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("BleedsClientInstaller.icon.ico"))
                    {
    ...
    [ComVisible(true)]
    public class BleedsClientBackend
    {
        ...
        public BleedsClientBackend(LauncherForm form, WebView2 webView)
        {
            ...
            _distDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Bleeds Client", "dist");
        }
        ...
        public async Task<bool> IsInjected(string path)
        {
            return await Task.Run(() => {
                ...
                return Directory.Exists(appDir) && File.Exists(pkgPath) && File.ReadAllText(pkgPath).Contains("\"bleeds-client\"");
            });
        }
        ...
        private async Task EnsureDistAsync()
        {
            ...
            if (embeddedPairs.Count > 0)
            {
                SetProgress(5, "Extracting bundled client files...");
                Directory.CreateDirectory(_distDir);
                for (int i = 0; i < embeddedPairs.Count; i++)
                {
                    var (fileName, resName) = embeddedPairs[i];
                    var destPath = Path.Combine(_distDir, fileName);
        ...
        private void DoInject(string resPath)
        {
            ...
            if (Directory.Exists(appDir))
            {
                ...
                if (File.Exists(pkg) && File.ReadAllText(pkg).Contains("\"bleeds-client\"")) {
                    WriteLoader(appDir);
            ...
            // Delete fake app.asar created by Vencord/OpenAsar (< 1 MB)
            if (File.Exists(appAsar) && new FileInfo(appAsar).Length < 1000000)
                RetryFileOp(() => File.Delete(appAsar));

            // Move real app.asar to backup
        ...
        private void DoUninject(string resPath)
        {
            ...
            if (Directory.Exists(appDir))
            {
                var pkg = Path.Combine(appDir, "package.json");
                if (File.Exists(pkg) && File.ReadAllText(pkg).Contains("\"bleeds-client\""))
                    RetryFileOp(() => Directory.Delete(appDir, true));
            }

            SetProgress(50, "Restoring original files...");
            // Delete fake app.asar before restoring
            if (File.Exists(appAsar) && new FileInfo(appAsar).Length < 1000000)
                RetryFileOp(() => File.Delete(appAsar));
            ...
            {
                if (!File.Exists(appAsar))
                    RetryFileOp(() => File.Move(backup, appAsar));
                else
                    RetryFileOp(() => File.Delete(backup)); // Discord updated, backup is stale
            }

            SetProgress(70, "Cleaning up assets...");
            // 3. Clean up Bleeds Client-specific assets (folders we added)
        ...
        private void WriteLoader(string appDir)
        {
            ...
            // _distDir = %LOCALAPPDATA%\Bleeds Client\dist\
            ...
            File.WriteAllText(Path.Combine(appDir, "package.json"), "{\"name\":\"bleeds-client\",\"main\":\"index.js\"}");
            File.WriteAllText(Path.Combine(appDir, "index.js"),
                $"// Bleeds Client Injector\n" +
                $"\"use strict\";\n" +
                $"const fs = require('fs');\n" +
                ...
                $"if (!fs.existsSync(patcherPath)) throw new Error('[Bleeds] patcher.js not found. Expected at: ' + primary);\n" +
                $"require(patcherPath);\n"
            );
        }
        ...
        private void KillDiscord(string resPath)
        {
            ...
            // Kill main process and any crash/helper sub-processes
            string[] procNames = { procName, procName + "CrashService", procName + "Helper" };
            ...
            // Wait long enough for the OS to release all file handles
            System.Threading.Thread.Sleep(2500);
        }

        // Retry a file action up to maxAttempts times, waiting retryDelayMs between each
        private void RetryFileOp(Action op, int maxAttempts = 5, int retryDelayMs = 600)
        {
            Exception last = null;
